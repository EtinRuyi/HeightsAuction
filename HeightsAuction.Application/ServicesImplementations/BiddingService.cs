...
namespace HeightsAuction.Application.ServicesImplementations
{
    public class BiddingService : IBiddingService
    {
        ...
        public async Task<ApiResponse<BidResponseDto>> AddBidAsync(string userId, AddBidRequestDto requestDto)
        {
            try
            {
                ...
                if (user == null)
                {
                    return ApiResponse<BidResponseDto>.Failed(false, "User does not exist", 404, new List<string> { });
                }
                ...
                {
                    return ApiResponse<BidResponseDto>.Failed(false, "Invalid bidding room or the room has finished", 400, new List<string> { });
                }
                ...
                {
                    return ApiResponse<BidResponseDto>.Failed(false, "User has already placed a bid on this item", 400, new List<string> { });
                }
            ...
            catch (Exception ex)
            {
                ...
                return ApiResponse<BidResponseDto>.Failed(false, "Error occurred while adding a bid", 500, new List<string> { });
            }
        }
